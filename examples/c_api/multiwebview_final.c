/**
 * Multi-WebView example - FINAL VERSION
 * Creates 4 WebViews in a 2x2 grid
 */
...
int main() {
    printf("=== Wry Multi-WebView (Final) ===\n");
    printf("Version: %s\n\n", wry_version());
    ...
    for (int i = 0; i < 4; i++) {
        ...
        bounds.width = half_width;
        bounds.height = half_height;
        
        printf("Setting bounds: x=%.0f, y=%.0f, w=%.0f, h=%.0f\n", bounds.x, bounds.y, bounds.width, bounds.height);
        if (wry_webview_set_bounds(webview, &bounds) != WRY_OK) {
            printf("ERROR: Failed to set bounds: %s\n", wry_last_error_message());
        } else {
            printf("OK: Bounds set\n");
        }
